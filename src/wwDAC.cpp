...
void DACClass::begin(uint32_t period) {
	...
	dacc_set_power_save(dac, 1, 1);             //*** Woody --war (0, 0)  mit 1, 1 verschwindet das Rauschen zwischen einzelnen Stücken.

	// DAC refresh/startup timings:
	// refresh        - 0x08 (1024*8 dacc clocks)
	// max speed mode -    0 (disabled)
	// startup time   - 0x10 (1024 dacc clocks)
	dacc_set_timing(dac, 0x08, 0, DACC_MR_STARTUP_1024);   //original: DACC_MR_STARTUP_1024  - DACC_MR_STARTUP_1920
	...
	//dacc_enable_channel(dac, 1);                 //*** Woody only uses dac0 so we can leave DAC 1 off  ***
...
//	PIO_Configure(g_APinDescription[DAC1].pPort,                //*** Woody only uses dac0 so we can leave DAC 1 off  ***
			// g_APinDescription[DAC1].ulPinType,               //*** Woody only uses dac0 so we can leave DAC 1 off  ***
			// g_APinDescription[DAC1].ulPin,                   //*** Woody only uses dac0 so we can leave DAC 1 off  *** 
			// g_APinDescription[DAC1].ulPinConfiguration);     //*** Woody only uses dac0 so we can leave DAC 1 off  ***
...
void DACClass::end() {
	...
	// dacc_disable_channel(dac, 1);                        //*** Woody only uses dac0 so we can leave DAC 1 off  ***
}
...
size_t DACClass::queueBuffer(const uint32_t *buffer, size_t size) {  // here we will use only the half of the buffer -ra
...
void DACClass::onService() {                                // DAC Interruptroutine 
	uint32_t sr = dac->DACC_ISR;
	
	if (sr & DACC_ISR_ENDTX) {
		// There is a free slot, enqueue data
		#ifdef DEBUGint
	    SerialUSB.println("Interrupt asks for data");	      //ra	
		#endif
		dacc_disable_interrupt(dac, DACC_IDR_ENDTX);
		if (cb)
			cb(cbData);
...
void DACC_ISR_HANDLER(void) {                              // DAC Interrupt handler
	DAC.onService();
}
